...
def _return_arg_is_valid(arg):
    is_char = len(arg) == 1 and (arg.isnumeric() is False)
    return is_char or arg in special_keys

def keyboard(*args) -> str:
    for arg in args:
        if not _return_arg_is_valid(arg):
            raise Exception(f'\'{arg}\' is neither a special keyword nor a key.')

    return return_tag('-'.join(args))

def click(button: Optional[str]='Any', on: Optional[str]=None, repetitions: Optional[int]=1) -> str:
    bind_string = 'Button'
    ...
    if repetitions < 1 or repetitions > 3:
        raise ArithmeticError("Number of click repetitions must be greater than 0 and less than 4.")
    else:
